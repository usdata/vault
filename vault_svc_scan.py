#!/usr/bin/env python3
"""
Scan a HashiCorp Vault Enterprise cluster (tested target: 1.16.28) for KV
secrets that contain at least one data field key beginning with "svc".
...
paths, reads each secret, and reports the path when any field key matches svc*.

Output: CSV with columns
    namespace, kv_path, matched_keys
(`matched_keys` is the svc* field name(s) found at that path — drop the column
if you only want the two you asked for.)
...
Usage:
    ...
    python3 vault_svc_scan.py                 # writes svc_kv_paths.csv
    python3 vault_svc_scan.py -o out.csv      # custom output file
    python3 vault_svc_scan.py --prefix svc    # change the match prefix
    python3 vault_svc_scan.py --insecure      # skip TLS verification (lab only)
    python3 vault_svc_scan.py --ns team-a/    # start at a sub-namespace
"""
...
def api(path, namespace="", list_op=False, ssl_ctx=None):
    ...
    # illegal in a raw URL (e.g. "connections/ svc-foo"). Percent-encode the
...
def main():
    ap = argparse.ArgumentParser(description="Find KV secrets with svc* field keys across all Vault namespaces.")
    ap.add_argument("-o", "--output", default="svc_kv_paths.csv", help="CSV output file (default: svc_kv_paths.csv)")
    ap.add_argument("--prefix", default="svc", help="field-key prefix to match (default: svc)")
    ...
    def do_read(ns, mount, version, rel):
        ...
        with counter_lock:
            counter["secrets"] += 1
        matched = sorted(k for k in fields if k.startswith(prefix))
        if matched:
            ns_label = ns if ns else "root"
            full_path = mount + rel  # e.g. "app1/kv/svc"
            with rows_lock:
